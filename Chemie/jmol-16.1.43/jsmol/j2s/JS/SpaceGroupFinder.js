...
Clazz.load (["JU.P3"], "JS.SpaceGroupFinder", ["JU.BS", "$.Lst", "$.PT", "JS.SpaceGroup", "$.Symmetry", "$.SymmetryOperation", "JU.BSUtil", "$.SimpleUnitCell", "JV.FileManager"], function () {
...
var sg = null;
var setNew = (isAssign && xyzList != null && this.nAtoms == 0 && (uc == null || uc.getSpaceGroup () == null || "P1".equals ((uc.getSpaceGroup ()).getName ())));
var name;
var basis;
if (setNew) {
...
uc = JS.SpaceGroupFinder.createCompatibleUnitCell (sg, unitCellParams);
basis =  new JU.BS ();
name = sg.asString ();
...
c$.createCompatibleUnitCell = Clazz.defineMethod (c$, "createCompatibleUnitCell", 
 function (sg, params) {
var sym;
var n = JU.PT.parseInt (sg.intlTableNumber);
var isHexGroup = JS.SpaceGroupFinder.isHexagonal (n, null);
if (n <= 2 || isHexGroup && JS.SpaceGroupFinder.isHexagonal (-1, params)) {
} else if (isHexGroup) {
params =  Clazz.newFloatArray (-1, [params[0], params[0], params[2], 90, 90, 120]);
} else if (n >= 195) {
params =  Clazz.newFloatArray (-1, [params[0], params[0], params[0], 90, 90, 90]);
} else if (n >= 75) {
params =  Clazz.newFloatArray (-1, [params[0], params[0], params[2], 90, 90, 90]);
} else if (n >= 16) {
params =  Clazz.newFloatArray (-1, [params[0], params[1], params[2], 90, 90, 90]);
} else {
switch (sg.uniqueAxis) {
case 'a':
params =  Clazz.newFloatArray (-1, [params[0], params[1], params[1], 90, params[4], params[5]]);
break;
case 'b':
params =  Clazz.newFloatArray (-1, [params[0], params[1], params[0], params[3], 90, params[5]]);
break;
case 'c':
params =  Clazz.newFloatArray (-1, [params[0], params[0], params[0], params[3], params[4], 90]);
break;
default:
break;
}
}sym =  new JS.Symmetry ().setUnitCell (params, false);
sym.setSpaceGroupTo (sg);
return sym;
}, "JS.SpaceGroup,~A");
c$.isHexagonal = Clazz.defineMethod (c$, "isHexagonal", 
 function (n, params) {
return (n < 1 ? params[0] == params[1] && params[3] == 90 && params[4] == 90 && params[5] == 120 : n >= 143 && n <= 194);
}, "~N,~A");
