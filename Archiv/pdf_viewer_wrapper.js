...
import { ZoomBehavior } from "chrome-extension://mhjfbmdgcfjbbpaeojofohoefgiehjai/browser_api.js";
export {
  BrowserApi,
  ZoomBehavior,
} from "chrome-extension://mhjfbmdgcfjbbpaeojofohoefgiehjai/browser_api.js";
  ...
  <custom-style>
    <style>
      html {
        ...
        --flat-focus-shadow-color: rgba(255, 69, 48, 0.4) !important;
    ...
    <style>
      ...
      @media (prefers-color-scheme: dark) {
        html {
          ...
          --cr-iph-anchor-highlight-color: rgba(
            var(--google-grey-100-rgb),
            0.1
          );
      ...
      html {
        ...
        --cr-container-shadow-margin: calc(
          -1 * var(--cr-container-shadow-height)
        );
...
class FocusRow {
  ...
  getElements() {
    return Array.from(this.root.querySelectorAll("[focus-type]")).map(
      FocusRow.getFocusableElement
    );
  }
  ...
  onKeydown_(e) {
    ...
    const isShiftTab =
      !e.altKey && !e.ctrlKey && !e.metaKey && e.shiftKey && e.key === "Tab";
...
class CrActionMenuElement extends PolymerElement {
  ...
  showAtPosition(config) {
    ...
    if (openedByKey) {
      const firstSelectableItem = this.querySelector(
        SELECTABLE_DROPDOWN_ITEM_QUERY
      );
  ...
  return html`<!--_html_template_start_-->
    <style>
      ...
      :host-context([dir="rtl"]):host(
          :not([dir="ltr"]):not([multiple-icons_])
        ) {
        transform: scaleX(-1);
      }
...
class CrIconButtonElement extends PolymerElement {
  constructor() {
    super();
...
function pinchScaleRatio(event, prevEvent) {
  const distance1 = distance$1(prevEvent);
  const distance2 = distance$1(event);
  return distance1 === 0 ? null : distance2 / distance1;
}
function distance$1(event) {
...
class SwipeDetector {
  constructor(element) {
    ...
    this.element_.addEventListener("touchend", this.onTouchEnd_.bind(this), {
      passive: true,
    });
    this.element_.addEventListener("touchcancel", () => this.onTouchCancel_(), {
      passive: true,
    });
  }
  ...
  onTouchEnd_(event) {
    ...
    if (
      elapsedTime <= SWIPE_TIMER_INTERVAL_MS &&
      Math.abs(distX) >= SWIPE_X_DIST_MIN &&
      Math.abs(distY) <= SWIPE_Y_DIST_MAX
    ) {
      const direction =
        distX > 0 ? SwipeDirection.LEFT_TO_RIGHT : SwipeDirection.RIGHT_TO_LEFT;
...
class ActiveZoomManager extends ZoomManager {
  ...
  onPdfZoomChange() {
    ...
    this.changingBrowserZoom_ = this.setBrowserZoomFunction_(viewportZoom).then(
      () => {
...
class EmbeddedZoomManager extends ZoomManager {
  onBrowserZoomChange(newZoom) {
    ...
    this.browserZoom = newZoom;
    this.getEventTarget().dispatchEvent(
      new CustomEvent("update-zoom-from-browser", {
        detail: oldZoom,
      })
    );
  }
...
class Viewport {
  constructor(container, sizer, content, scrollbarWidth, defaultZoom) {
    ...
    this.scrollContent_ = new ScrollContent(
      this.window_,
      sizer,
      content,
      scrollbarWidth
    ...
    this.gestureDetector_
      .getEventTarget()
      .addEventListener("pinchstart", (e) => this.onPinchStart_(e));
    this.gestureDetector_
      .getEventTarget()
      .addEventListener("pinchupdate", (e) => this.onPinchUpdate_(e));
    this.gestureDetector_
      .getEventTarget()
      .addEventListener("pinchend", (e) => this.onPinchEnd_(e));
    this.gestureDetector_
      .getEventTarget()
      .addEventListener("wheel", (e) => this.onWheel_(e));
    this.swipeDetector_ = new SwipeDetector(content);
    this.swipeDetector_
      .getEventTarget()
      .addEventListener("swipe", (e) => this.onSwipe_(e));
  ...
  getLayoutOptions() {
    return this.documentDimensions_
      ? this.documentDimensions_.layoutOptions
      : undefined;
  }
  getViewportRect_() {
  ...
  setZoomManager(manager) {
    ...
    this.tracker_.add(this.zoomManager_.getEventTarget(), "set-zoom", (e) =>
      this.setZoom(e.detail)
    );
    this.tracker_.add(
      this.zoomManager_.getEventTarget(),
      "update-zoom-from-browser",
  ...
  getLastPageInViewport_(viewportRect) {
    ...
    return getIntersectionArea(viewportRect, nextPage) > 0
      ? pageAtY + 1
      : pageAtY;
  }
  ...
  computeFittingZoom_(pageDimensions, fitWidth, fitHeight) {
    ...
    zoom = this.computeFittingZoomGivenDimensions_(
      fitWidth,
      fitHeight,
      windowWithScrollbars.width,
      windowWithScrollbars.height,
      pageDimensions.width,
      pageDimensions.height
    );
  ...
  pageUpDownSpaceHandler_(e, formFieldFocused) {
    ...
    if (this.isPagedMode_()) {
      ...
      const scrollOffset =
        (isDown ? 1 : -1) * this.size.height * MIN_FRACTION_TO_STEP_WHEN_PAGING;
      this.setPosition(
        {
          x: this.position.x,
  ...
  onPinchUpdate_(e) {
    ...
    window.requestAnimationFrame(() => {
      ...
      this.mightZoom_(() => {
        ...
        this.pinchPhase_ =
          direction === "out"
            ? PinchPhase.UPDATE_ZOOM_OUT
            : PinchPhase.UPDATE_ZOOM_IN;
        const scaleDelta = startScaleRatio / this.prevScale_;
        if (this.firstPinchCenterInFrame_ != null) {
          this.pinchPanVector_ = vectorDelta(
            center,
            this.firstPinchCenterInFrame_
          );
        }
        const needsScrollbars = this.documentNeedsScrollbars(
  ...
  onPinchEnd_(e) {
    window.requestAnimationFrame(() => {
      this.mightZoom_(() => {
        const { center: center, startScaleRatio: startScaleRatio } = e.detail;
  ...
  onSwipe_(e) {
    ...
    if (
      (e.detail === SwipeDirection.RIGHT_TO_LEFT && !isRTL()) ||
...
class PluginController {
  ...
  set isActive(isActive) {
    ...
    this.eventTarget_.dispatchEvent(
      new CustomEvent(PluginControllerEventType.IS_ACTIVE_CHANGED, {
  ...
  afterZoom() {
    ...
    const zoom = this.viewport_.getZoom();
    const layoutOptions = this.viewport_.getLayoutOptions();
    const pinchVector = this.viewport_.pinchPanVector || {
      x: 0,
      y: 0,
    };
    const pinchCenter = this.viewport_.pinchCenter || {
      x: 0,
      y: 0,
    };
    const pinchPhase = this.viewport_.pinchPhase;
  ...
  bindMessageHandler(port) {
    ...
    for (const { message: message, transfer: transfer } of delayedMessages) {
      this.plugin_.postMessage(message, transfer);
    }
  }
  ...
  static getInstance() {
    return instance$1 || (instance$1 = new PluginController());
  }
}
let instance$1 = null;
/**
    ...
    <svg>
      <defs>
        <g id="account-child-invert" viewBox="0 0 48 48">
          <path
            d="M24 4c3.31 0 6 2.69 6 6s-2.69 6-6 6-6-2.69-6-6 2.69-6 6-6z"
          ></path>
...
styleMod$6.register("pdf-shared");
...
class ViewerBookmarkElement extends PolymerElement {
  ...
  depthChanged_() {
    ...
    this.$.item.style.paddingInlineStart = this.depth * BOOKMARK_INDENT + "px";
  }
  onClick_() {
    if (this.bookmark.page != null) {
      if (this.bookmark.zoom != null) {
        this.fire_("change-zoom", {
...
function getTemplate$d() {
  return html`<!--_html_template_start_--><style include="pdf-shared">
      :host {
        display: block;
...
customElements.define(
  ViewerDocumentOutlineElement.is,
  ViewerDocumentOutlineElement
);
const template$1 = html`<iron-iconset-svg size="24" name="pdf">
  <svg>
    <defs>
      ...
      <g id="eraser">
        ...
        ></path>
        <polygon points="17.26 18 15.26 20 21.96 20 21.96 18"></polygon>
      </g>
      <g id="fit-to-height">
        <path
          fill-rule="evenodd"
      ...
      <g id="highlighter">
        <path
          d="M10.22,9.49 L4.31,15.49 C3.54,16.29 3.61,17.54 4.39,18.34 L0.77,22 L6.45,22 L7.19,21.25 C7.97,22.06 9.14,22.11 9.92,21.3 L15.88,15.25 L10.22,9.49 L10.22,9.49 Z"
        ></path>
        <path
          style="fill:var(--pen-tip-fill)"
          d="M22.68,5.49 L19.86,2.62 C19.08,1.82 17.79,1.78 17.02,2.58 L11.27,8.43 L16.93,14.18 L22.62,8.4 C23.39,7.59 23.45,6.29 22.68,5.49 L22.68,5.49 Z"
        ></path>
        <path
          style="fill:var(--pen-tip-border)"
          d="M18.4,3c0.3,0,0.5,0.1,0.7,0.3L22,6.2c0.4,0.4,0.4,1.1-0.1,1.5l-5,5.1l-4.3-4.3l5.1-5.2 C17.9,3.1,18.1,3,18.4,3 M18.4,2c-0.5,0-1,0.2-1.4,0.6l-5.8,5.9l5.7,5.8l5.7-5.8c0.8-0.8,0.8-2.1,0.1-2.9l-2.8-2.9 C19.5,2.2,18.9,2,18.4,2L18.4,2z"
        ></path>
      </g>
      <g id="marker">
        <polygon points="3 17.25 3 21 6.74 21 14.28 13.47 10.53 9.72"></polygon>
        <path
          style="fill:var(--pen-tip-fill)"
...
class ViewerDownloadControlsElement extends PolymerElement {
  ...
  static get properties() {
    return {
      ...
      downloadHasPopup_: {
        type: String,
        computed:
          "computeDownloadHasPopup_(hasEdits," + "hasEnteredAnnotationMode)",
      },
      menuOpen_: {
        type: Boolean,
        reflectToAttribute: true,
  ...
  showDownloadMenu_() {
    ...
    this.dispatchEvent(
      new CustomEvent("download-menu-shown-for-testing", {
        bubbles: true,
        composed: true,
      })
    );
...
customElements.define(
  ViewerDownloadControlsElement.is,
  ViewerDownloadControlsElement
);
const template = html`
  <custom-style>
    <style>
...
// Copyright 2019 The Chromium Authors
const CrButtonElementBase = mixinBehaviors(
  [PaperRippleBehavior],
  PolymerElement
);
        ...
        for (const entry of entries) {
          ...
          this.sides_.forEach((side) => {
            if (target === this.intersectionProbes_.get(side)) {
              this.dropShadows_
                .get(side)
                .classList.toggle("has-shadow", entry.intersectionRatio === 0);
            }
          });
        }
      };
      return new IntersectionObserver(callback, {
        root: this.getContainer_(),
        threshold: 0,
      });
    }
    ...
    <style include="cr-hidden-style cr-icons">
      ...
      :host ::slotted([slot="button-container"]) {
        ...
        padding-inline-end: var(
          --cr-dialog-button-container-padding-horizontal,
          16px
        );
      ...
      #cr-container-shadow-top {
        border-bottom: var(
          --cr-dialog-body-border-top,
          var(--transparent-border)
        );
      }
      #cr-container-shadow-bottom {
        border-bottom: var(
          --cr-dialog-body-border-bottom,
          var(--transparent-border)
        );
      }
...
class CrDialogElement extends CrDialogElementBase {
  ...
  showModal() {
    ...
    this.open = true;
    this.dispatchEvent(
      new CustomEvent("cr-dialog-open", {
        bubbles: true,
        composed: true,
      })
    );
  ...
  onKeypress_(e) {
    ...
    const accept =
      ...
      e
        .composedPath()
        .some((el) => el.tagName === "CR-INPUT" && el.type !== "search");
    if (!accept) {
      return;
    }
...
styleMod$3.appendChild(
  html`
    <template>
      <style>
        :host {
          --cr-radio-button-checked-color: var(--cr-link-color) !important;
          --cr-radio-button-checked-ripple-color: rgba(
            var(--cr-link-color-rgb),
            0.2
          ) !important;
        }
        :focus {
          outline-color: var(--flat-focus-shadow-color);
          outline-width: 3px;
        }
        cr-checkbox {
          --cr-checkbox-checked-box-color: var(--cr-link-color) !important;
          --cr-checkbox-ripple-checked-color: var(--cr-link-color) !important;
        }
        button,
        h1,
        h2,
...
styleMod$3.register("br-shared-style");
const styleMod$2 = document.createElement("dom-module");
styleMod$2.appendChild(
  html`
    <template>
      <style include="cr-hidden-style br-shared-style cr-icons">
        :host,
        html {
...
customElements.define(
  ViewerPasswordDialogElement.is,
  ViewerPasswordDialogElement
);
    ...
    <div id="thumbnails" hidden$="[[!isPluginActive_]]" role="tablist">
      ...
      >
        <viewer-thumbnail
          ...
          is-active="[[isActivePage_(item, activePage)]]"
          page-number="[[item]]"
        >
        </viewer-thumbnail>
      </template>
...
class ViewerThumbnailBarElement extends PolymerElement {
      ...
      (entries) => {
        entries.forEach((entry) => {
          ...
          this.pluginController_
            .requestThumbnail(thumbnail.pageNumber)
            .then((response) => {
  ...
  onDomChange_() {
    this.shadowRoot
      .querySelectorAll("viewer-thumbnail")
      .forEach((thumbnail) => {
  ...
  onFocus_() {
    ...
    const activeThumbnail = this.shadowRoot.querySelector(
  ...
  onKeydown_(e) {
    switch (e.key) {
      case "Tab":
        ...
        const lastThumbnail = this.shadowRoot.querySelector(
...
(function (UserAction) {
  ...
  UserAction[(UserAction["FOLLOW_BOOKMARK_FIRST"] = 9)] =
    "FOLLOW_BOOKMARK_FIRST";
  UserAction[(UserAction["FOLLOW_BOOKMARK"] = 10)] = "FOLLOW_BOOKMARK";
  UserAction[(UserAction["PAGE_SELECTOR_NAVIGATE_FIRST"] = 11)] =
    "PAGE_SELECTOR_NAVIGATE_FIRST";
  UserAction[(UserAction["PAGE_SELECTOR_NAVIGATE"] = 12)] =
    "PAGE_SELECTOR_NAVIGATE";
  UserAction[(UserAction["SAVE_FIRST"] = 13)] = "SAVE_FIRST";
  UserAction[(UserAction["SAVE"] = 14)] = "SAVE";
  UserAction[(UserAction["SAVE_WITH_ANNOTATION_FIRST"] = 15)] =
    "SAVE_WITH_ANNOTATION_FIRST";
  UserAction[(UserAction["SAVE_WITH_ANNOTATION"] = 16)] =
    "SAVE_WITH_ANNOTATION";
  UserAction[(UserAction["PRINT_FIRST"] = 17)] = "PRINT_FIRST";
  UserAction[(UserAction["PRINT"] = 18)] = "PRINT";
  UserAction[(UserAction["ENTER_ANNOTATION_MODE_FIRST"] = 19)] =
    "ENTER_ANNOTATION_MODE_FIRST";
  UserAction[(UserAction["ENTER_ANNOTATION_MODE"] = 20)] =
    "ENTER_ANNOTATION_MODE";
  UserAction[(UserAction["EXIT_ANNOTATION_MODE_FIRST"] = 21)] =
    "EXIT_ANNOTATION_MODE_FIRST";
  UserAction[(UserAction["EXIT_ANNOTATION_MODE"] = 22)] =
    "EXIT_ANNOTATION_MODE";
  UserAction[(UserAction["ANNOTATE_STROKE_TOOL_PEN_FIRST"] = 23)] =
    "ANNOTATE_STROKE_TOOL_PEN_FIRST";
  UserAction[(UserAction["ANNOTATE_STROKE_TOOL_PEN"] = 24)] =
    "ANNOTATE_STROKE_TOOL_PEN";
  UserAction[(UserAction["ANNOTATE_STROKE_TOOL_ERASER_FIRST"] = 25)] =
    "ANNOTATE_STROKE_TOOL_ERASER_FIRST";
  UserAction[(UserAction["ANNOTATE_STROKE_TOOL_ERASER"] = 26)] =
    "ANNOTATE_STROKE_TOOL_ERASER";
  UserAction[(UserAction["ANNOTATE_STROKE_TOOL_HIGHLIGHTER_FIRST"] = 27)] =
    "ANNOTATE_STROKE_TOOL_HIGHLIGHTER_FIRST";
  UserAction[(UserAction["ANNOTATE_STROKE_TOOL_HIGHLIGHTER"] = 28)] =
    "ANNOTATE_STROKE_TOOL_HIGHLIGHTER";
  UserAction[(UserAction["ANNOTATE_STROKE_DEVICE_TOUCH_FIRST"] = 29)] =
    "ANNOTATE_STROKE_DEVICE_TOUCH_FIRST";
  UserAction[(UserAction["ANNOTATE_STROKE_DEVICE_TOUCH"] = 30)] =
    "ANNOTATE_STROKE_DEVICE_TOUCH";
  UserAction[(UserAction["ANNOTATE_STROKE_DEVICE_MOUSE_FIRST"] = 31)] =
    "ANNOTATE_STROKE_DEVICE_MOUSE_FIRST";
  UserAction[(UserAction["ANNOTATE_STROKE_DEVICE_MOUSE"] = 32)] =
    "ANNOTATE_STROKE_DEVICE_MOUSE";
  UserAction[(UserAction["ANNOTATE_STROKE_DEVICE_PEN_FIRST"] = 33)] =
    "ANNOTATE_STROKE_DEVICE_PEN_FIRST";
  UserAction[(UserAction["ANNOTATE_STROKE_DEVICE_PEN"] = 34)] =
    "ANNOTATE_STROKE_DEVICE_PEN";
  UserAction[(UserAction["TWO_UP_VIEW_ENABLE_FIRST"] = 35)] =
    "TWO_UP_VIEW_ENABLE_FIRST";
  UserAction[(UserAction["TWO_UP_VIEW_ENABLE"] = 36)] = "TWO_UP_VIEW_ENABLE";
  UserAction[(UserAction["TWO_UP_VIEW_DISABLE_FIRST"] = 37)] =
    "TWO_UP_VIEW_DISABLE_FIRST";
  ...
  UserAction[(UserAction["THUMBNAIL_NAVIGATE_FIRST"] = 45)] =
    "THUMBNAIL_NAVIGATE_FIRST";
  UserAction[(UserAction["THUMBNAIL_NAVIGATE"] = 46)] = "THUMBNAIL_NAVIGATE";
  UserAction[(UserAction["SAVE_ORIGINAL_ONLY_FIRST"] = 47)] =
    "SAVE_ORIGINAL_ONLY_FIRST";
  ...
  UserAction[(UserAction["TOGGLE_SIDENAV_FIRST"] = 53)] =
    "TOGGLE_SIDENAV_FIRST";
  UserAction[(UserAction["TOGGLE_SIDENAV"] = 54)] = "TOGGLE_SIDENAV";
  UserAction[(UserAction["SELECT_SIDENAV_THUMBNAILS_FIRST"] = 55)] =
    "SELECT_SIDENAV_THUMBNAILS_FIRST";
  UserAction[(UserAction["SELECT_SIDENAV_THUMBNAILS"] = 56)] =
    "SELECT_SIDENAV_THUMBNAILS";
  UserAction[(UserAction["SELECT_SIDENAV_OUTLINE_FIRST"] = 57)] =
    "SELECT_SIDENAV_OUTLINE_FIRST";
  UserAction[(UserAction["SELECT_SIDENAV_OUTLINE"] = 58)] =
    "SELECT_SIDENAV_OUTLINE";
  UserAction[(UserAction["TOGGLE_DISPLAY_ANNOTATIONS_FIRST"] = 59)] =
    "TOGGLE_DISPLAY_ANNOTATIONS_FIRST";
  UserAction[(UserAction["TOGGLE_DISPLAY_ANNOTATIONS"] = 60)] =
    "TOGGLE_DISPLAY_ANNOTATIONS";
...
function getTemplate$4() {
  return html`<!--_html_template_start_--><style
      include="pdf-shared cr-hidden-style cr-shared-style"
    >
      :host {
        ...
        <table>
          ...
          <tr>
            ...
            <td class="value" id="fast-web-view">
              [[getFastWebViewValue_('Yes', 'No', documentMetadata.linearized)]]
            </td>
          </tr>
        </table>
      </div>
...
customElements.define(
  ViewerPropertiesDialogElement.is,
  ViewerPropertiesDialogElement
);
/**
@license
Copyright (c) 2015 The Polymer Project Authors. All rights reserved.
  ...
  _template: html`
    <style>
      ...
      :host(:not([disabled])) #primaryProgress.indeterminate {
        ...
        animation: indeterminate-bar
          var(--paper-progress-indeterminate-cycle-duration, 2s) linear infinite;
      }
    ...
    <div id="progressContainer">
      <div
        ...
        hidden$="[[_hideSecondaryProgress(secondaryRatio)]]"
      ></div>
  ...
  return html`<!--_html_template_start_--><style include="pdf-shared">
    ...
    <div id="toolbar">
      ...
      <div id="center">
        ...
        <cr-icon-button
          id="fit"
          iron-icon="[[fitToButtonIcon_]]"
          title="[[getFitToButtonTooltip_('Fit to page',
                                        'Fit to width',
                                        fittingType_)]]"
          aria-label="[[getFitToButtonTooltip_('Fit to page',
                                             'Fit to width',
                                             fittingType_)]]"
      ...
      <div id="end">
        ...
        <cr-icon-button
          id="print"
          iron-icon="cr:print"
          hidden="[[!printingEnabled]]"
          title="Print"
          aria-label="Print"
          on-click="onPrintClick_"
        >
        </cr-icon-button>
        <cr-icon-button
          id="more"
    ...
    <cr-action-menu id="menu" on-open-changed="onMoreOpenChanged_">
      <button
        id="two-page-view-button"
        class="dropdown-item"
        on-click="toggleTwoPageViewClick_"
        role="checkbox"
      ...
      <button
        id="show-annotations-button"
        class="dropdown-item"
        on-click="toggleDisplayAnnotations_"
        role="checkbox"
...
class ViewerToolbarElement extends PolymerElement {
  ...
  toggleDisplayAnnotations_() {
    ...
    this.dispatchEvent(
      new CustomEvent("display-annotations-changed", {
  ...
  toggleTwoPageViewClick_() {
    ...
    this.dispatchEvent(
      new CustomEvent("two-up-view-changed", {
  ...
  forceFit(fittingType) {
    this.fittingType_ =
      fittingType === FittingType.FIT_TO_WIDTH
  ...
  fitToggle() {
    const newState =
      this.fittingType_ === FittingType.FIT_TO_PAGE
        ...
        : FittingType.FIT_TO_PAGE;
    this.dispatchEvent(
      new CustomEvent("fit-to-changed", {
...
class PdfNavigator {
  ...
  navigate(urlString, disposition) {
    ...
    switch (disposition) {
      case WindowOpenDisposition.CURRENT_TAB:
        whenDone = this.paramsParser_
          .getViewportFromUrlParams(url.href)
          .then(this.onViewportReceived_.bind(this));
        break;
  ...
  guessUrlWithoutScheme_(url) {
    if (
      !this.originalUrl_ ||
      this.originalUrl_.protocol === "mailto:" ||
    ...
    if (!url.startsWith(".")) {
      ...
      const domainName =
        domainSeparatorIndex === -1 ? url : url.substr(0, domainSeparatorIndex);
  ...
  async getViewportFromUrlParams(url) {
    ...
    if (params.page === undefined && urlParams.has("nameddest")) {
      ...
      if (data.namedDestinationView) {
        Object.assign(
          params,
          this.parseNameddestViewParam_(data.namedDestinationView)
        );
      }
      return params;
    }
...
function getTemplate() {
  return html`<!--_html_template_start_--><style
      include="pdf-viewer-shared-style cr-hidden-style"
    >
      :host {
      ...
      #sidenav-container {
        overflow: hidden;
        transition: transform 250ms cubic-bezier(0.6, 0, 0, 1), visibility 250ms;
        visibility: visible;
        width: var(--viewer-pdf-sidenav-width);
      }
      ...
      #sidenav-container[closed] {
        ...
        transition: transform 0.2s cubic-bezier(0.6, 0, 0, 1), visibility 0.2s,
          width 0s 0.2s;
        visibility: hidden;
        width: 0;
      }
    ...
    <viewer-toolbar
      ...
      on-rotate-left="rotateCounterclockwise"
      on-print="onPrint_"
      on-save="onToolbarSave_"
      hidden
    >
    </viewer-toolbar>
    ...
    <div id="container">
      ...
      <div id="main">
        ...
        <div
          id="content-focus-rectangle"
          hidden$="[[!documentHasFocus_]]"
        ></div>
      </div>
    </div>
...
class ViewportScroller {
  ...
  calculateVelocity_(event) {
    const x =
      Math.min(
        Math.max(-event.offsetX, event.offsetX - this.plugin_.offsetWidth, 0),
        MAX_DRAG_SCROLL_DISTANCE
      ) * Math.sign(event.offsetX);
    const y =
      Math.min(
        Math.max(-event.offsetY, event.offsetY - this.plugin_.offsetHeight, 0),
        MAX_DRAG_SCROLL_DISTANCE
...
class PdfViewerBaseElement extends PolymerElement {
  ...
  createPlugin_() {
    ...
    if (this.browserApi.getStreamInfo().embedded) {
      plugin.setAttribute(
        "top-level-url",
        this.browserApi.getStreamInfo().tabUrl
      );
    } else {
  ...
  initInternal(browserApi, scroller, sizer, content) {
    ...
    this.paramsParser = new OpenPdfParamsParser((destination) =>
      PluginController.getInstance().getNamedDestination(destination)
    );
    const defaultZoom =
    ...
    this.viewport_.setUserInitiatedCallback((userInitiated) =>
      this.setUserInitiated_(userInitiated)
    );
    ...
    this.zoomManager_ = ZoomManager.create(
      this.browserApi.getZoomBehavior(),
      () => this.viewport_.getZoom(),
      (zoom) => this.browserApi.setZoom(zoom),
      this.browserApi.getInitialZoom()
    );
    this.viewport_.setZoomManager(this.zoomManager_);
    this.browserApi.addZoomEventListener((zoom) =>
      this.zoomManager_.onBrowserZoomChange(zoom)
    );
    this.viewportScroller = new ViewportScroller(
      this.viewport_,
      this.plugin_,
      window
    );
  ...
  updateProgress(progress) {
    if (progress === -1) {
      ...
      this.paramsParser
        .getViewportFromUrlParams(this.originalUrl)
  ...
  handleStrings(strings) {
    ...
    const presetZoomFactors = JSON.parse(
      loadTimeData.getString("presetZoomFactors")
    );
...
class PdfViewerElement extends PdfViewerBaseElement {
  constructor() {
    ...
    this.sidenavCollapsed_ = Boolean(
      Number.parseInt(
  ...
  init(browserApi) {
    ...
    this.toolbarEnabled_ = this.paramsParser.shouldShowToolbar(
      this.originalUrl
    );
  ...
  handleStrings(strings) {
    ...
    this.pdfAnnotationsEnabled_ = loadTimeData.getBoolean(
    ...
    this.zoomBounds_.max = Math.round(
      presetZoomFactors[presetZoomFactors.length - 1] * 100
    );
  }
  handleScriptingMessage(message) {
    ...
    switch (message.data.type.toString()) {
      case "getSelectedText":
        this.pluginController_
          .getSelectedText()
          .then(this.handleSelectedTextReply.bind(this));
        break;
      case "print":
...
export {
  ...
  SwipeDirection,
  UserAction,
  ViewerBookmarkElement,
  ViewerDocumentOutlineElement,
  ViewerDownloadControlsElement,
  ViewerPageSelectorElement,
  ViewerPasswordDialogElement,
  ViewerPdfSidenavElement,
  ViewerPropertiesDialogElement,
  ViewerThumbnailBarElement,
  ViewerThumbnailElement,
  ViewerToolbarElement,
  Viewport,
  ViewportScroller,
